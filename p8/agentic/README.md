...
|---|---------------|-------------|-------------|
| 1 | **Agents load from DB or schema folder as fallback.** Schema folder can be any path (e.g. `/tmp/schema`). | `P8_SCHEMA_DIR=/tmp/schema p8 chat --agent my-agent` | `curl -X POST http://localhost:8000/chat/test-1 -H 'x-agent-schema-name: my-agent' -H 'Content-Type: application/json' -d '{"thread_id":"t1","run_id":"r1","messages":[{"id":"m1","role":"user","content":"hello"}],"tools":[],"context":[],"forwarded_props":{},"state":{}}'` |
...
| 3 | **Streaming children are non-blocking.** Child agent events interleave with parent via `_merged_event_stream`. | `p8 chat --agent general` then ask it to delegate: "ask the sample agent about X" | `curl -N http://localhost:8000/chat/test-2 -H 'x-agent-schema-name: general' ...` — observe `child_content` SSE events interleaved with parent events |
